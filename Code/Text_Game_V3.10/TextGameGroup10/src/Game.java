...
public class Game {
	...
	void executeToken(Environment env, Token token){
		...
		if(token.command.equalsIgnoreCase("ANSWER")) {this.executeAnswer(env, token);return;}
	...
	/*display riddle of the actor in this room*/
	void executeAsk() {
	...
	/*ANSWER by itself will print the question to the screen again. 
	 * ANSWER userAnswer will be used for a string comparison to the NPC¡¯s riddleAnswer. 
	 * If they match, the riddle is solved; 
	 * else the user is teleported to the entrance of the environment. */
