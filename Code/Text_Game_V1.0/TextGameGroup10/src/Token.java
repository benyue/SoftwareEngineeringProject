...
public class Token {
	...
	static Token extractToken(String input){
		Token token = new Token(); //as long as it's "newed" here, the token is not equal to "null"
		String[] tokenParts = input.trim().split(" ",2); 
		//allow space before and after, more tolerable to incorrect input
		if(tokenParts.length == 1){
			...
			if((tokenParts[0].trim().equalsIgnoreCase("GO") 
				...
				token.direction = abbreviateDir(tokenParts[1].trim().toUpperCase()); //
				return token;
			}
		}
		return null; 
